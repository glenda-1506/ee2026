...
//    integer prev_brac_count = 0;
    ...
    character_bitmap char_bitmap_inst (
        .char(current_char),
        .row(current_row),
...
//            if (selected_key == KEY_DELETE || check_validity(selected_key, last_selected_key, cursor_pos)) begin
...
//                    char_bitmap = get_character_bitmap(char_buffer[i], y_addr - 30);
//                    if (char_bitmap[7 - (x_addr - (86 - i*8))]) begin
...
//                    char_bitmap = get_character_bitmap(char_buffer[i], y_addr - 38);
//                    if (char_bitmap[7 - (x_addr - (86 - (i - 8)*8))]) begin
//                        pixel_data <= 16'hFFFF;  // Set pixel color to white
//                    end
//                end
//            end
//        end
        ...
        for (i = 0; i < 16; i = i + 1) begin
            if (i < 10) begin
                if (x_addr >= (78 - i*8) && x_addr < (86 - i*8) && y_addr >= 30 && y_addr < 38) begin
                    current_char <= char_buffer[i];                // Set current character
                    current_row <= y_addr - 30;                    // Set current row
                    #1; // small delay to allow char_bitmap_wire to update (use non-blocking with FSM in real design)
